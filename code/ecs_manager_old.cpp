...
//     auto res = component_maps[typ]->get(comp_id);
//     if(res == nullptr){
//         throw NullPtrException("Could not find element of id " + std::to_string(comp_id) + " and type " + get_str(typ));
//     }
//     return res;
// }
// Component* ECS_manager::get_component_from_entity(int16_t entity_id, std::type_index typ){
//     if(component_maps[typ]->id_map.size()==0){
//         throw NullPtrException("Trying to get component from empty components list of " + get_str(typ));
//     }
//     auto res = component_maps[typ]->get_with_entity_id(entity_id);
//     if(res == nullptr){
//         throw NullPtrException("Could not find element with entity id " + std::to_string(entity_id) + "on list of " + get_str(typ));
//     }
//     return res;
// }
// std::vector<Component*> ECS_manager::get_components_from_entity(int16_t entity_id, std::type_index typ){
//     if(component_maps[typ]->id_map.size()==0){
//         throw NullPtrException("Trying to get components from empty components list!");
//     }
//     auto comp = component_maps[typ]->get_first();
//     if(!comp->allows_multiplicity){
//         throw IllegalMultipleComponents(comp->name, entity_id);
//     }
...
// }
// Component* ECS_manager::get_on(int16_t comp_id, std::type_index ftyp, std::type_index typ){
//     auto entity_id = get_entity(comp_id, ftyp);
//     return get_component_from_entity(entity_id, typ);
// }
// std::vector<Component*> ECS_manager::get_all_on(int16_t comp_id, std::type_index ftyp, std::type_index typ){
//     auto entity_id = get_entity(comp_id, ftyp);
//     return get_components_from_entity(entity_id, typ);
// }
...
// }
// int16_t ECS_manager::get_singleton_comp_id(std::type_index typ){
//     if(component_maps[typ]->id_map.size() == 0){
//         throw NullPtrException("Trying to get singleton id of typ " + get_str(typ) + ", which hasn't yet been added!");
//     }
//     return component_maps[typ]->get_first()->component_id;
// }
...
//     auto res = component_maps[typ]->id_map.find(component_id);
//     if(res == component_maps[typ]->id_map.end()){
//         throw NullPtrException("Trying to get entity id of component " + std::to_string(component_id) + ", but this component has not yet been added, or is not of type " + get_str(typ));
//     }
//     return res->second;
// }
