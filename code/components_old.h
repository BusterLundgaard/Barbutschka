...
//     virtual Component* get_with_entity_id(int16_t entity_id)=0;
...
//     virtual Component* get_with_entity_id(int16_t entity_id) override;
...
//     auto id_iter = id_map.find(c->component_id);
//     if(id_iter != id_map.end()){
//         throw IllegalMultipleAdd(c->component_id);
//     }
...
// template <typename T>
// Component* ComponentMap<T>::get_with_entity_id(int16_t entity_id) {
//     for(int i = 0; i < components.size(); i++){
//         auto as_comp = static_cast<Component*>(&components[i]);  
//         if(id_map[as_comp->component_id] == entity_id){
//             return as_comp;
